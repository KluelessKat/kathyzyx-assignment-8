...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.scatter(X[y == 0, 0], X[y == 0, 1], label='C0', c='blue')
        plt.scatter(X[y == 1, 0], X[y == 1, 1], label='C1', c='red')

        # Implement: Calculate and store logistic loss
        # raise NotImplementedError("Calculate and store logistic loss")
        y_prob = model.predict_proba(X)
        N = X.shape[0]
        loss = -np.sum(y * np.log(y_prob[:,1]) + (1 - y) * np.log(y_prob[:,0])) / N
    ...
    plt.ylabel("Beta0")
    plt.scatter(shift_distances, beta0_list)
    ...
    plt.xlabel("Shift Distance")
    plt.ylabel("Beta1")
    plt.scatter(shift_distances, beta1_list)
    ...
    plt.xlabel("Shift Distance")
    plt.ylabel("Beta2")
    plt.scatter(shift_distances, beta2_list)
    ...
    plt.ylim(-2, 0)
    plt.scatter(shift_distances, 
                [b1 / b2 for b1, b2 in zip(beta1_list, beta2_list)])
    ...
    plt.scatter(shift_distances, 
                [b0 / b2 for b0, b2 in zip(beta0_list, beta2_list)])
    ...
    plt.scatter(shift_distances, loss_list)
    ...
    plt.scatter(shift_distances, margin_widths)
